...
export const getNotes: GetNotes = async() => {
  ...
  if(isEmpty(notes)) {
    console.info('Nenhuma nota foi encontrada!')
...
export const readyNote = async (filename: string) => {
  const rootDir = getRootDir()
...
export const createNote: CreateNote = async () => {
  ...
  if(parentDir !== rootDir) {
    await dialog.showMessageBox({
      ...
      message: `All notes must be saved under ${rootDir}, foi criado ${parentDir}. Avoid using other directories!`,
    })
